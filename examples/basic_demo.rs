...
use core::{
    Address, Hash, Transaction, Block, BlockHeader, 
    KeccakPatriciaTrie, Trie
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    trie.insert(alice_addr.as_bytes(), bincode::encode_to_vec(&1000u64, bincode::config::standard())?)?;
    trie.insert(bob_addr.as_bytes(), bincode::encode_to_vec(&500u64, bincode::config::standard())?)?;
    ...
    if let Some(alice_balance_bytes) = trie.get(alice_addr.as_bytes())? {
        let (alice_balance, _): (u64, usize) = bincode::decode_from_slice(&alice_balance_bytes, bincode::config::standard())?;
